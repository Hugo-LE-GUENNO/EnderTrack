...
class StackViewer {
  ...
  _renderViewport() {
    ...
    if (sizeZ > 1) {
      if (this._projecting) {
        slidersHtml += `<div class="stack-slider-row" style="display:flex; align-items:center; gap:4px; margin-bottom:6px; padding:2px 4px; border-radius:3px;"><button id="stackZBtn" onclick="EnderTrack.StackViewer._toggleProjection()" oncontextmenu="event.preventDefault(); EnderTrack.StackViewer._showProjectionMenu(event)" style="border:none; background:var(--active-element); color:var(--text-selected); cursor:pointer; font-size:9px; padding:1px 4px; border-radius:2px;" title="Clic: d\u00e9sactiver, Clic droit: type">Z</button><span style="font-size:9px; color:var(--text-general); flex:1;">Projection ${(this._projType || 'max').toUpperCase()}</span></div>`;
      } else {
