...
def _get_strip(pin_name, total_pixels):
    ...
    if pin_name in _strips:
        ...
        if total_pixels > strip['total']:
            ...
            if bp:
                strip['obj'] = neopixel.NeoPixel(bp, total_pixels, auto_write=False, pixel_order=neopixel.GRB)
                strip['total'] = total_pixels
        return strip['obj']
    else:
        bp = _get_board_pin(pin_name)
        if bp:
            obj = neopixel.NeoPixel(bp, total_pixels, auto_write=False, pixel_order=neopixel.GRB)
